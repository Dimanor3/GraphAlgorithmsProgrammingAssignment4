...
public class FloydWarshall {
	...
	public FW(Graph G) 
	{
		...
		{
			fwMatrix[vertices.indexOf(eachEdge.getBeginLocation())][vertices.indexOf(eachEdge.getEndLocation())] 
			= eachEdge.getWeightOfEdge();
		}
	}
		...
		for (int k = 0; k < numV; k++) {
            for (int h = 0; h < numV; h++) {
                if (fwMatrix[k][h] != POSITIVE_INF) {
					SB.append("\nShortest Path from: " + vertices.get(k).getDolphinName() + " to: " 
					+ vertices.get(h).getDolphinName() + "\n");
